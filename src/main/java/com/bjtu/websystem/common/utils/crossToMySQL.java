...
public class crossToMySQL {
    public static void main(String[] args) throws IOException, SQLException {
        ...
        String path="D:\\返校文件\\毕业设计\\ete软件\\EtE软件\\EtE软件\\ete1_dataset\\ete1_dataset\\cross.csv";
...
//        String name=new BufferedReader(new FileReader(new File(path))).readLine();
        String name = "id,name,longtitude,latitude,type,car_sum,car_start_time,car_start_interval,passenger_sum,car_passenger_sum,truck_sum,motor_sum,capacity";
        //name就是表格列的名称
        reader.readHeaders();
        int len=reader.getHeaders().length; // 13列
//        System.out.println(len);
        //len表示的是有几个列
        while(reader.readRecord()){  //整个while就是为了组装成为 插入语句的形式
            String tmp="insert into cross_tbl("+name+")values("+reader.get(0);
